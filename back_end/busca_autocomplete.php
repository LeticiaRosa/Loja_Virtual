...
if ($acao == 'FECHA_CAIXA') :
	...
	$sql = "SELECT CAX.NOME AS NOME_CAIXA,EMP.NOME AS NOME_EMPRESA,CAX.NOME_MAQUINA, US.NOME_USUARIO, 
	CX.DATA_ABERTURA,CX.HORA_ABERTURA,CX.VALOR_INICIAL FROM CONTROLE_CAIXA AS CX 	LEFT OUTER JOIN CAIXA AS CAX	
	ON CAX.ID_CAIXA=CX.ID_CAIXA	LEFT OUTER JOIN EMPRESA AS EMP	ON EMP.ID_EMPRESA=CAX.ID_EMPRESA	
	LEFT OUTER JOIN USUARIO  AS US	ON US.ID_USUARIO=CX.ID_USUARIO_ABERTURA	WHERE CAX.NOME_MAQUINA=?
	AND  CX.DATA_FECHAMENTO IS NULL AND CX.HORA_ABERTURA=(SELECT MAX(AUX.HORA_ABERTURA) FROM CONTROLE_CAIXA AS AUX  WHERE AUX.ID_CAIXA=CX.ID_CAIXA)";
	$stm = $conexao->prepare($sql);
	$stm->bindValue(1,  $parametro  );
	$stm->execute();
	$dados = $stm->fetchAll(PDO::FETCH_OBJ);
...
if ($acao == 'lista_caixa') :
	$sql = "SELECT CAX.NOME AS NOME_CAIXA, EMP.NOME AS NOME_EMPRESA, CAX.NOME_MAQUINA, CAX.STATUS, CASE WHEN ( CX.DATA_ABERTURA IS NOT NULL AND CX.DATA_FECHAMENTO IS NULL) THEN 'Caixa Aberto' ELSE 'Caixa Fechado' END AS STATUS_CAIXA,
	coalesce(US.NOME_USUARIO,'') AS USUARIO_ABERTURA, coalesce(DATE_format(CX.DATA_ABERTURA, '%d-%m-%Y'),'') AS DATA_ABERTURA , 
	coalesce(USF.NOME_USUARIO,'') AS USUARIO_FECHAMENTO, coalesce(DATE_format(CX.DATA_FECHAMENTO, '%d-%m-%Y'),'') AS DATA_FECHAMENTO,
 	CAX.ID_CAIXA FROM CAIXA AS CAX 
 	LEFT OUTER JOIN CONTROLE_CAIXA AS CX ON CAX.ID_CAIXA=CX.ID_CAIXA 
 	LEFT OUTER JOIN EMPRESA AS EMP ON EMP.ID_EMPRESA=CAX.ID_EMPRESA 
	LEFT OUTER JOIN USUARIO  AS US    ON US.ID_USUARIO=CX.ID_USUARIO_ABERTURA 
...
if ($acao == 'BUSCA_DADOS_FECHAMENTO') :
	$sql = "SELECT COUNT(1)AS QUANTIDADE, 
	TIPO_VENDA,SUM(TOTAL_DESCONTO) AS TOTAL 	
	FROM VENDAS  AS VEN	LEFT OUTER JOIN CAIXA AS CX	
	ON CX.ID_CAIXA=VEN.ID_CAIXA	WHERE DATA_VENDA>=(SELECT MAX(AUX.DATA_ABERTURA)
	 FROM CONTROLE_CAIXA AS AUX  WHERE AUX.ID_CAIXA=VEN.ID_CAIXA AND DATA_FECHAMENTO 
	 IS NULL )	AND CX.NOME_MAQUINA=?	GROUP BY TIPO_VENDA
     UNION 
 SELECT COUNT(1)AS QUANTIDADE, 
	'Retiradas'TIPO_VENDA,SUM(VALOR_RETIRADA) AS TOTAL 	
	FROM retirada_caixa  AS RT	LEFT OUTER JOIN CAIXA AS CX	
	ON CX.ID_CAIXA=RT.ID_CAIXA	
    WHERE RT.DATA_RETIRADA>=(SELECT MAX(AUX.DATA_ABERTURA)
